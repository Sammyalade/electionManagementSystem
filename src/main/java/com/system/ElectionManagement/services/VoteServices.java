...
public class VoteServices implements VoteServiceImpl {
    ...
    public AddVoteResponse castVote(CastVoteRequest voteRequest) {
            ...
            Optional<Election> election = electionRepository.findById(voteRequest.getElectionId());
            if(election.isEmpty()) {
                throw new ElectionNotStartedException("No election started");
            }
            Election gotten = election.get();
            var existingVote = voteRepository.findByVoterAndElection(voteRequest.getId(), gotten.getId());
